...
import {
	getControlPosition,
	createCoreData
} from '@/libs/draggableUtils'
import { getDocumentDir } from '@/libs/DOM'
...
.vue-box-item.cssTransforms {
	transition-property: transform;
	left: 0;
	right: auto;
}
.vue-box-item.cssTransforms.render-rtl {
	left: auto;
	right: 0;
}
.vue-box-item.resizing {
	opacity: 0.6;
	z-index: 3;
}
.vue-box-item.vue-draggable-dragging {
	transition: none;
	z-index: 3;
}
.vue-box-item.vue-box-placeholder {
	background: red;
	opacity: 0.2;
...
.vue-box-item > .vue-resizable-handle {
	position: absolute;
	width: 20px;
	height: 20px;
	bottom: 0;
...
.vue-box-item > .vue-rtl-resizable-handle {
	bottom: 0;
	left: 0;
...
.vue-box-item.disable-userselect {
	user-select: none;
}
...
.vue-box-item:not(.vue-box-placeholder) {
